// ================================================================
// MOYTRACK PRO v4.0  —  app.js
// Filtrlar · Saqlashda SMS · Avtomatik shablon · LocalStorage DB
// ================================================================
...
document.getElementById('theme-btn').addEventListener('click', () => { cfg.theme = cfg.theme === 'dark' ? 'light' : 'dark'; saveCfg(); applyTheme(); });
...
function loadDashboard() {
  ...
  const notif = [...uc, ...wc];
  uel.innerHTML = notif.length ? notif.map(ciHTML).join('') : '<div class="empty"><div class="ei">🎉</div><p>Hammasi yaxshi!</p></div>';
  addCIE(uel);

  const ael = document.getElementById('all-cars-list');
  ael.innerHTML = allCars.length ? allCars.map(ciHTML).join('') : '<div class="empty"><div class="ei">🚗</div><p>Hali mashina qo\'shilmagan</p></div>';
  addCIE(ael);
}
...
function addCIE(el) {
  el.querySelectorAll('.ci').forEach(e => e.addEventListener('click', () => { curCar = allCars.find(c => c.id == e.dataset.id); openModal(); }));
}
...
function loadCarsGrid(q = '') {
  ...
  grid.querySelectorAll('.cc').forEach(e => e.addEventListener('click', () => { curCar = allCars.find(c => c.id == e.dataset.id); openModal(); }));
}
...
function setupHintListeners() {
  const pairs = [
    ['current-km', 'oil-change-km',       'oil-hint',          () => [
      parseFloat(document.getElementById('current-km')?.value)||0,
      parseFloat(document.getElementById('oil-change-km')?.value)||0,
      oilInt(document.getElementById('oil-name')?.value)
    ]],
    ['current-km', 'antifreeze-km',        'antifreeze-hint',   () => [
      parseFloat(document.getElementById('current-km')?.value)||0,
      parseFloat(document.getElementById('antifreeze-km')?.value)||0,
      parseFloat(document.getElementById('antifreeze-interval')?.value)||30000
    ]],
    ['current-km', 'gearbox-km',           'gearbox-hint',      () => [
      parseFloat(document.getElementById('current-km')?.value)||0,
      parseFloat(document.getElementById('gearbox-km')?.value)||0,
      parseFloat(document.getElementById('gearbox-interval')?.value)||50000
    ]],
    ['current-km', 'air-filter-km',        'air-filter-hint',   () => [
      parseFloat(document.getElementById('current-km')?.value)||0,
      parseFloat(document.getElementById('air-filter-km')?.value)||0,
      parseFloat(document.getElementById('air-filter-interval')?.value)||15000
    ]],
    ['current-km', 'cabin-filter-km',      'cabin-filter-hint', () => [
      parseFloat(document.getElementById('current-km')?.value)||0,
      parseFloat(document.getElementById('cabin-filter-km')?.value)||0,
      parseFloat(document.getElementById('cabin-filter-interval')?.value)||15000
    ]],
    ['current-km', 'oil-filter-km',        'oil-filter-hint',   () => [
      parseFloat(document.getElementById('current-km')?.value)||0,
      parseFloat(document.getElementById('oil-filter-km')?.value)||0,
      parseFloat(document.getElementById('oil-filter-interval')?.value)||10000
    ]],
  ];
...
function resetAddCarForm() {
  ...
  ['oil-hint','antifreeze-hint','gearbox-hint','air-filter-hint','cabin-filter-hint','oil-filter-hint',
   'anti-pill-hint','gear-pill-hint'].forEach(clearChip);
...
function fillTemplate(tmpl, car, svcType) {
  ...
  if (svcType === 'antifreeze')   { lastKm = car.antifreeze_km;   interval = car.antifreeze_interval; }
  if (svcType === 'gearbox')      { lastKm = car.gearbox_km;      interval = car.gearbox_interval; }
  if (svcType === 'air_filter')   { lastKm = car.air_filter_km   || car.total_km; interval = car.air_filter_interval   || 15000; }
  if (svcType === 'cabin_filter') { lastKm = car.cabin_filter_km || car.total_km; interval = car.cabin_filter_interval || 15000; }
  if (svcType === 'oil_filter')   { lastKm = car.oil_filter_km   || car.total_km; interval = car.oil_filter_interval   || 10000; }
...
function buildSaveSmsText(car, checkedKeys) {
  ...
  const servicesList = checkedKeys.map(k => {
    const m = SVC_META[k];
...
function openModal() {
  ...
  document.getElementById('modal-services').innerHTML =
    ...
    svcBlock(gU,  '🟢 Karobka',     curCar.total_km - curCar.gearbox_km,     curCar.gearbox_interval) +
  ...
  // Show/hide oil brand select based on service type
  const svcSel = document.getElementById('modal-svc-type');
  svcSel.onchange = () => {
    document.getElementById('modal-oil-wrap').style.display = svcSel.value === 'oil' ? '' : 'none';
  };
  document.getElementById('modal-oil-wrap').style.display = '';

  loadHistory();
  const hint = document.getElementById('sms-hint');
  if (smsConfig.enabled && smsConfig.api_url) {
    hint.innerHTML = '💬 SMS yoqilgan — almashtirish keyin {car_name} va {car_number} avtomatik qo\'shiladi';
  ...
  el.innerHTML = [...hist].reverse().map(log => {
    ...
    return `<div class="hi${isT ? ' htd' : ''}">
      ...
      <div>
        ...
        <div class="hdate">${ds}${isT ? ' · ' + timeStr : ''}</div>
      </div>
    </div>`;
  }).join('');
}

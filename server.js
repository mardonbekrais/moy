...
// ── 8. TOKEN TEKSHIRISH ─────────────────────────────────────────
// POST /api/sms/check-token
// Body: { token }
// DevSMS tokenini tekshiradi — balans va holat qaytaradi
app.post('/api/sms/check-token', async (req, res) => {
  const { token } = req.body || {};
  if (!token) return res.status(400).json({ ok: false, error: 'token majburiy' });

  try {
    // DevSMS user info endpoint
    ...
    if (!r.ok || data.error) {
      return res.json({
        ...
        message: "Token noto\u02BCg\u02BCri yoki muddati o\u02BCtgan",
      });
    }

    const balance = data.balance ?? data.credit ?? data.sms_count ?? '—';
    const login   = data.login  ?? data.username ?? data.email ?? '—';

    console.log(`✅ Token tekshirildi: ${login}, balans: ${balance}`);
    res.json({
      ok:      true,
      message: `Balans: ${balance} so'm | Login: ${login}`,
      balance,
      login,
      raw:     data,
    });
  } catch (e) {
    console.error('Token tekshirish xatosi:', e.message);
    res.status(502).json({ ok: false, error: "DevSMS ga ulanib bo\u02BClmadi: " + e.message });
  }
});

// ── 9. TO'G'RIDAN SMS YUBORISH ─────────────────────────────────
// POST /api/sms/send
